...
namespace Geometry{
	...
	struct Line{
		...
		Line(Point a=Point(0,0),Vector b=Vector(0,0)):P(a),V(b){ ang=Angle(V); }
	...
	//下面的两个判断函数均允许线段在端点处相交，如需判断严格相交需要将IsLSI的返回换成乘积为-1
...
inline int HpI(Line L[],int n,Point Poly[]){
	sort(L+1,L+n+1);
	int f,l;
	...
	For(i,2,n){
		...
		if(f<l)p[l-1]=LI(q[l].P,q[l].V,q[l-1].P,q[l-1].V);
	}
...
inline Circle GetCircle(Point A,Point B,Point C){
	Circle Cir;
	Cir.O = LI((A + B) / 2,Vector(-(B - A).y,(B - A).x),(B + C) / 2,Vector(-(C - B).y,(C - B).x));
	Cir.r = Length(Cir.O - A);
	return Cir;
}
inline Circle MCC(Point P[],int n){
	random_shuffle(P + 1,P + n + 1);
	Circle C = Circle(P[1],0);
	For(i,2,n){
		if(dcmp(Length(P[i] - C.O) - C.r) <= 0)continue;
		C = Circle(P[i],0);
		For(j,1,i - 1){
			if(dcmp(Length(P[j] - C.O) - C.r) <= 0)continue;
			C.O=(P[i] + P[j]) / 2,C.r = Length(C.O - P[i]);
			For(k,1,j - 1){
				if(dcmp(Length(P[k] - C.O) - C.r) <= 0)continue;
