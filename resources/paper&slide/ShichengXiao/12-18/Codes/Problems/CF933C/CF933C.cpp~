...
namespace Geometry{
	...
	//此处以及下方所有直线的表达方式，是用一个点和一个从这个点出发的向量来表示的。
...
int main(){
#ifndef ONLINE_JUDGE
	freopen("CF933C.in","r",stdin);
	freopen("CF933C.out","w",stdout);
#endif
	int n=read(),cnt=0;
	For(i,1,n)C[i].O.x=read(),C[i].O.y=read(),C[i].r=read();
	int e=0,v=0,c=0;Point P1,P2;
	For(i,1,n)fa[i]=i;
	For(i,1,n){
		int top=0;
		For(j,1,n){
			if(i==j)continue;
			int sum=CI(C[i],C[j],P1,P2);
			if(sum)Merge(i,j);
			if(sum>=1)A[++cnt]=P1,B[++top]=P1;
			if(sum>=2)A[++cnt]=P2,B[++top]=P2;
		}
		sort(B+1,B+top+1);
		e+=unique(B+1,B+top+1)-B-1;
	}
	sort(A+1,A+cnt+1);
	v=unique(A+1,A+cnt+1)-A-1;
	For(i,1,n)if(find(i)==i)c++;
	printf("%d\n",e-v+c+1);
	return 0;
}
