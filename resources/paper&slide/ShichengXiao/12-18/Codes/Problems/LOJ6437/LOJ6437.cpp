...
namespace Geometry{
	...
	bool IsPoS(Point P,Point A,Point B){ //Is_Point_on_Segment
		if(A.x>B.x)swap(A,B);
		if(dcmp(P.x-A.x)<0 || dcmp(P.x-B.x)>0 || abs((B.y-A.y)/(B.x-A.x)-(B.y-P.y)/(B.x-P.x))>1e-6)return 0;
		return 1;
	}
...
const int N=2e2+10,M=5e2+10;
Point Poly[M];
double Ang[M<<1];
Circle C[N];
bool Check(Point P,Point A,Point B){ //Is_Point_on_Segment
	if(A.x>B.x)swap(A,B);
	if(dcmp(P.x-A.x)<0 || dcmp(P.x-B.x)>0)return 0;
	return 1;
}
int main(){
#ifndef ONLINE_JUDGE
	freopen("LOJ6437.in","r",stdin);
	freopen("LOJ6437.out","w",stdout);
#endif
	int n=read(),m=read();
	For(i,1,n){
		double x=read(),y=read(),Dis=Length(Vector(x,y));
		C[i]=Circle(Point(0.0,0.0),Dis);
	}
	For(i,1,m)Poly[i].x=read(),Poly[i].y=read();
	Poly[m+1]=Poly[1];
	double ans=0;
	For(i,1,n){
		int sum=0;
		For(j,1,m){
			Point P1,P2;
			int cnt=CLI(C[i],Poly[j],Poly[j+1],P1,P2);
			if(cnt>=1 && Check(P1,Poly[j],Poly[j+1]))Ang[++sum]=Angle(P1);
			if(cnt>=2 && Check(P2,Poly[j],Poly[j+1]))Ang[++sum]=Angle(P2);
		}
		if(sum==0){
			if(IsPiP(Point(0,C[i].r),Poly,m))ans++;
			continue;
		}
		sort(Ang+1,Ang+sum+1);
		Point P;
		For(j,1,sum-1){
			P=C[i].point((Ang[j]+Ang[j+1])/2.0);
			if(IsPiP(P,Poly,m)==1)ans+=(Ang[j+1]-Ang[j])/(2*Pi);
		}
		double ang=(Ang[sum]+Ang[1]+2*Pi)/2.0;
		if(dcmp(ang-Pi)==1)ang-=2*Pi;
		P=C[i].point(ang);
		if(IsPiP(P,Poly,m)==1)ans+=(Ang[1]-Ang[sum]+2*Pi)/(2*Pi);
	}
	printf("%.5lf\n",ans);
	return 0;
}
