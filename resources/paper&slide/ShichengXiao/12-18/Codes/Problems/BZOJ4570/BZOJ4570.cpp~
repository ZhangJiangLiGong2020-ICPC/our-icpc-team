...
int main(){
...
#endif
	int n=read();double Mx=0,My=0;
	For(i,1,n)P[i].x=read(),P[i].y=read(),Mx=max(Mx,P[i].x),My=max(My,P[i].y);
	P[++n]=(Point){0,0},P[++n]=(Point){0,My},P[++n]=(Point){Mx,0};
	sort(P+1,P+n+1);
	int cnt=0;
	For(i,1,n){
	...
	double ans=INF;
	For(i,1,cnt){
		if(!dcmp(Ans[i].x) || !dcmp(Ans[i].y))continue;
		double Liml,Limr;
		Liml=!dcmp(Ans[i-1].x-Ans[i].x) ? -INF : (Ans[i-1].y-Ans[i].y)/(Ans[i-1].x-Ans[i].x);
		Limr=!dcmp(Ans[i+1].x-Ans[i].x) ? -INF : (Ans[i+1].y-Ans[i].y)/(Ans[i+1].x-Ans[i].x);
		double val=-sqrt(Ans[i].y/Ans[i].x);
		if(val>=Liml && val<=Limr)ans=min(ans,Ans[i].x+Ans[i].y+2*sqrt(Ans[i].x*Ans[i].y));
		if(!dcmp(Ans[i].x-Ans[i+1].x) || !dcmp(Ans[i].y-Ans[i+1].y))continue;
		Point p=Point(Ans[i].x,Ans[i].y);Vector V=Vector(Ans[i+1].x-Ans[i].x,Ans[i+1].y-Ans[i].y);
		double sum=0;
		sum+=LI(p,V,Point(0,0),Vector(1,0)).x;
		sum+=LI(p,V,Point(0,0),Vector(0,1)).y;
		ans=min(ans,sum);
	}
	printf("%.4lf\n",ans);
	return 0;
