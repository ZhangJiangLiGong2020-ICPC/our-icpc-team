...
int main(){
#ifndef ONLINE_JUDGE
	freopen("UVA11168.in","r",stdin);
	freopen("UVA11168.out","w",stdout);
#endif
	int Case=0,T=read();
	while(T--){
		int n=read();Case++;
		...
		For(i,1,cnt-1){
			L[i].A=Ans[i+1].y-Ans[i].y;
			L[i].B=Ans[i].x-Ans[i+1].x;
			L[i].C=Ans[i+1].x*Ans[i].y-Ans[i].x*Ans[i+1].y;
		}
		double ans=INF;
		For(i,1,cnt-1){
			double Dis=abs(L[i].A*Sum1+L[i].B*Sum2+n*L[i].C)/sqrt(L[i].A*L[i].A+L[i].B*L[i].B);
			ans=min(ans,Dis);
		}
		printf("Case #%d: %.3lf\n",Case,ans/n);
	}
